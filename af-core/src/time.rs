  ...
  if #[cfg(feature = "postgres")] {
    ...
    impl<'a> pg::FromSql<'a> for Time {
      fn from_sql(ty: &pg::Type, raw: &'a [u8]) -> Result<Self, Box<dyn Error + Sync + Send>>{
        let inner: chrono::DateTime<chrono::Utc> = pg::FromSql::from_sql(ty, raw)?;

        Ok(Self(inner.with_timezone(&Tz::UTC)))
      }
    ...
    impl pg::ToSql for Time {
        ...
        Self: Sized,
      {
        if ty.oid() == pg::Type::TIMESTAMP.oid() {
          self.0.naive_local().to_sql(ty, out)
        } else {
          self.0.with_timezone(&chrono::Utc).to_sql(ty, out)
        }
      }
      ...
      fn to_sql_checked(&self, ty: &pg::Type, out: &mut bytes::BytesMut) -> Result<pg::IsNull, Box<dyn Error + Sync + Send>> {
        if ty.oid() == pg::Type::TIMESTAMP.oid() {
          self.0.naive_local().to_sql_checked(ty, out)
        } else {
          self.0.with_timezone(&chrono::Utc).to_sql_checked(ty, out)
        }
      }
    }
  }
